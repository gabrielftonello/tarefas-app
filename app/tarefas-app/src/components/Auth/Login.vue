<template>
  <v-card class="mx-auto my-12" max-width="400">
    <v-card-title>Entrar</v-card-title>
    <v-card-text>
      <v-form @submit.prevent="aoEnviar" ref="formulario">
        <v-text-field
            label="Email"
            v-model="email"
            :rules="regrasEmail"
            required
        ></v-text-field>
        <v-text-field
            label="Senha"
            v-model="senha"
            :rules="regrasSenha"
            type="password"
            required
        ></v-text-field>
        <v-btn type="submit" color="primary" :loading="carregando">Entrar</v-btn>
        <v-btn text @click="irParaRegistro" style="margin-left: 22px;">Cadastre-se</v-btn>
      </v-form>
      <v-alert v-if="erro" type="error" dense>{{ erro }}</v-alert>
    </v-card-text>
  </v-card>
</template>

<script>
import { mapActions } from 'vuex';

export default {
  data() {
    return {
      email: '',
      senha: '',
      regrasEmail: [
        v => !!v || 'O campo Email deve ser preenchido.',
        v => /.+@.+\..+/.test(v) || 'O Email deve ser válido.',
      ],
      regrasSenha: [
        v => !!v || 'O campo Senha é obrigatório.',
        v => v.length >= 6 || 'A senha deve ter pelo menos 6 caracteres.',
      ],
      carregando: false,
      erro: '',
    };
  },
  methods: {
    ...mapActions('auth', ['login']),
    async aoEnviar() {
      const valido = this.$refs.formulario.validate();
      if (valido) {
        this.carregando = true;
        this.erro = '';
        try {
          await this.login({ email: this.email, password: this.senha });
          this.$router.push({ name: 'Home' });
        } catch (err) {
          this.erro = err.response?.data?.message || 'Falha no login';
        } finally {
          this.carregando = false;
        }
      }
    },
    irParaRegistro() {
      this.$router.push({ name: 'Register' });
    },
  },
};
</script>

<style scoped>
</style>
